...
const money = (value: string | number, currency: string) => { const n = Number(value); return `${n >= 0 ? "+" : "−"}${Math.abs(n).toFixed(4)} ${currency}`; };
const qty = (value: string | number) => Number(value).toLocaleString(undefined, { maximumFractionDigits: 6 });
...
export default function Page() {
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.market}>
      <div><small>Mid price</small><strong>{latest ? qty((Number(latest.market.bids[0]!.price) + Number(latest.market.asks[0]!.price)) / 2) : "—"}</strong></div>
    ...
    <section className={styles.strategies}>
      {(["baseline", "jev", "control"] as const).map((id) => {
        ...
        return <article className={styles.card} key={id}>
          ...
          <div className={styles.metrics}><div><small>Inventory</small><strong>{qty(state?.inventory ?? "0")}</strong></div><div><small>Open offers</small><strong>{state?.offers.length ?? 0}</strong></div><div><small>Realized P&amp;L ({latest?.market.quote.currency ?? "quote"})</small><strong>{money(state?.realizedPnl ?? "0", latest?.market.quote.currency ?? "")}</strong></div><div><small>Total P&amp;L ({latest?.market.quote.currency ?? "quote"})</small><strong className={Number(snapshot?.totalPnl ?? 0) >= 0 ? styles.positive : styles.negative}>{money(snapshot?.totalPnl ?? "0", latest?.market.quote.currency ?? "")}</strong></div><div><small>XRPL fee model</small><strong>{qty(Number(state?.xrplFeeDrops ?? 0) / 1_000_000)} XRP</strong></div><div><small>Jev inference</small><strong>${Number(state?.jevCostUsd ?? 0).toFixed(6)}</strong></div></div>
