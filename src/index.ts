...
try {
  ...
  if (config.network === "mainnet") assertFreshMainnetDataDir(config.dataDir, config.source === "replay" ? "replay" : "mainnet");
  ...
  const replayHash = config.source === "replay" ? createHash("sha256").update(readFileSync(config.replayPath!)).digest("hex") : undefined;
  ...
  if (existing && (![3, 4, 5, EVENT_VERSION].includes((existing as any).schemaVersion) || (existing.network ?? "testnet") !== config.network || JSON.stringify(existing.base) !== base || JSON.stringify(existing.quote) !== quote || existing.source !== config.source || existing.seed !== (config.source === "synthetic" ? config.seed : undefined) || existing.replayHash !== replayHash || existing.replayIntervalMs !== (config.source === "replay" ? config.replayIntervalMs : undefined) || JSON.stringify(existing.assumptions) !== JSON.stringify(expectedAssumptions))) {
  ...
  const session = existing ? { ...existing, schemaVersion: EVENT_VERSION, assumptions } : { schemaVersion: EVENT_VERSION, sessionId: crypto.randomUUID(), startedAt: Date.now(), source: config.source, network: config.network, ...(config.source === "synthetic" ? { seed: config.seed } : {}), ...(replayHash ? { replayHash } : {}), ...(config.source === "replay" ? { replayIntervalMs: config.replayIntervalMs } : {}), base: config.base, quote: config.quote, assumptions };
  ...
  const source = config.source === "testnet" || config.source === "mainnet" ? new XrplMarketDataSource(trader.resumeLedger) : config.source === "synthetic" ? new SyntheticMarketDataSource(config.seed, trader.resumeLedger) : new ReplayMarketDataSource(trader.resumeLedger, config.replayPath, config.replayIntervalMs, config.network);
...
function parseOptions(args: string[]) {
  const allowed: Record<string, string> = { "--mode": "MODE", "--network": "NETWORK", "--source": "SOURCE", "--synthetic-seed": "SYNTHETIC_SEED", "--replay": "REPLAY_PATH", "--replay-interval-ms": "REPLAY_INTERVAL_MS", "--base": "BASE_CURRENCY", "--base-issuer": "BASE_ISSUER", "--quote": "QUOTE_CURRENCY", "--quote-issuer": "QUOTE_ISSUER", "--model": "MODEL", "--data-dir": "DATA_DIR", "--mainnet-ws-url": "XRPL_MAINNET_WS_URL", "--usd-to-quote-rate": "USD_TO_QUOTE_RATE" };
