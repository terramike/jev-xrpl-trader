...
try {
  ...
  if (existing && ((existing.network ?? "testnet") !== config.network || JSON.stringify(existing.base) !== base || JSON.stringify(existing.quote) !== quote || existing.source !== config.source || existing.seed !== (config.source === "synthetic" ? config.seed : undefined) || JSON.stringify(existing.assumptions) !== JSON.stringify(assumptions))) {
  ...
  const session = existing ?? { schemaVersion: 3 as const, sessionId: crypto.randomUUID(), startedAt: Date.now(), source: config.source, network: config.network, ...(config.source === "synthetic" ? { seed: config.seed } : {}), base: config.base, quote: config.quote, assumptions };
  ...
  const source = config.source === "testnet" || config.source === "mainnet" ? new XrplMarketDataSource() : config.source === "synthetic" ? new SyntheticMarketDataSource(config.seed, trader.resumeLedger) : new ReplayMarketDataSource(trader.resumeLedger);
...
function parseOptions(args: string[]) {
  const allowed: Record<string, string> = { "--mode": "MODE", "--network": "NETWORK", "--source": "SOURCE", "--synthetic-seed": "SYNTHETIC_SEED", "--replay": "REPLAY_PATH", "--base": "BASE_CURRENCY", "--base-issuer": "BASE_ISSUER", "--quote": "QUOTE_CURRENCY", "--quote-issuer": "QUOTE_ISSUER", "--model": "MODEL", "--data-dir": "DATA_DIR", "--mainnet-ws-url": "XRPL_MAINNET_WS_URL" };
