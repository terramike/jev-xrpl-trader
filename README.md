...
`bun run trader report` shows marked trading P&L and net quote-currency P&L separately. Net subtracts modeled XRP offer fees valued at the latest observed XRP/quote midpoint and modeled Jev USD cost converted using `USD_TO_QUOTE_RATE` (default assumption: 1 quote unit per USD). The report prints both conversions and the rate assumption. If the base is not native XRP, net remains unavailable rather than inventing a fee conversion. It also reports ledger counts/gaps, eligible direct-offer volume, unsupported cases, saved fill hashes and evidence, time holding XRP, peak long/short inventory, cumulative quote notional placed, and Jev abstention reasons. Holding time counts ledger-to-ledger intervals where inventory was nonzero; quote turnover counts created and replacement offers, not unchanged resting quotes.

The XRPL ledger reader serializes ledger processing, resumes from the last audited ledger, requests each validated ledger with expanded transactions, and reconciles all transaction metadata before reading that ledger's book. A transient server rate limit retries the same read-only request after its advertised delay (or a bounded backoff); a persistent failure, missing ledger or metadata, failed book request, empty book, or crossed book pauses the feed rather than publishing a partial or out-of-order event. Payments whose metadata does not expose direct target-book offer deltas are excluded; `delivered_amount` is not treated as DEX volume.
